...
1. Créez un dossier `backup` avec les sauvegardes de séquence au format `.mbz` si vous en avez.
...
2. Créez un fichier `.csv` avec la liste des séquences à créer ([voir example de fichier sequences.csv](./samples/sequences.csv))
    ...
    Les métiers du numerique,num,Niveau 1 / Niveau 2 / Niveau 3 / Niveau 4,2022-02-01,../../../../backup/les-metiers-du-numerique.mbz
    ```

    :::caution Points d'attention
    * shortname : le nom court d'une séquence doit être **unique sur tout le site** (mais pas le fullname) - pensez à établir une convention pour vos contributeurs ;
    ...
    * backupfile (facultatif) : le **chemin est relatif** à la position du script Moodle dans `admin/tool/uploadcourse/cli/` - si vos sauvegardes sont dans un dossier backup à la racine, il faut donc remonter de 4 niveaux d'abord `../../../../backup`.

3. Importez les cours en ligne de commande (notez le **chemin relatif du fichier csv** à la racine du projet) :

    ```
    php admin/tool/uploadcourse/cli/uploadcourse.php --mode=createnew --file=../../../../sequences.csv
    ```
...
2. Importez les utilisateurs (attention au **chemin du fichier csv**) :
    ...
    ```
    php admin/tool/uploaduser/cli/uploaduser.php --file=`pwd`/users.csv --delimiter_name=comma
    ```
...
## Attribution des rôles utilisateurs dans les catégories
...
1. Créez un fichier `.csv` avec les attributions des rôles par catégorie aux utilisateurs ([voir example de fichier users_categories.csv](./samples/users_categories.csv)).
    ...
    * role : le rôle à attribuer dans cette catégorie, soit manager pour un gestionnaire de site, coursecreator pour un contributeur, student pour un stagiaire ;
...
2. Attribuez les rôles (attention au **chemin du fichier csv**) :
    ...
    ```
    php theme/padplus/cli/assign_category_roles.php `pwd`/users_categories.csv
    ```


## Automisation des imports avec Ansible {#ansible-import}
...
1. Construisez votre dossier des données à importer avec les différents fichiers `.xml`, `.csv` et `.mbz`. Voici un exemple d'organisation :
    ...
    :::caution Chemins relatifs
    Vérifiez que le chemin d'accès relatif aux fichiers de sauvegarde est correct dans les fichiers csv de séquence. Le chemin est relatif au script Moodle `admin/tool/uploadcourse/cli/uploadcourse.php`, et commence donc généralement par `../../../../` si vos données sont à la racine du projet.
    :::

2. Dans votre fichier d'inventaire Ansible, rajouter une section `import_data`. Notez qu'il est possible de scinder vos descriptions des catégories et des séquences en plusieurs fichiers :
...
Il est possible de n'exécuter qu'une partie des tâches d'import avec les [tags Ansibles](https://docs.ansible.com/ansible/latest/user_guide/playbooks_tags.html#selecting-or-skipping-tags-when-you-run-a-playbook), dans l'ordre des étapes :

* categories : création de l'arborescence des catégories
* courses : création de séquences dans l'arborescence (avec ou sans sauvegardes de séquences existantes)
* users : création des comptes utilisateurs et inscription optionnelle aux séquences
* users_categories : attribution des rôles utilisateurs dans les catégories

Par exemple, pour n'exécuter que la création des catégories et des séquences :

`ansible-playbook import_data.yml -i my-pad.hosts.yml -u root --tags [categories, courses]`

Notez cependant que chaque étape dépend des précédentes.
:::

:::info Tâches optionnelles
Le playbook définit aussi des tâches qui ne sont pas exécutées par défaut mais uniquement à la demande :
- tag `remove-misc-category` : supprimer la catégorie "Divers" par défaut de Moodle (identifiant 1)
- tag `fix-single-activity-page` : forcer le type d'activité "page" pour **toutes les séquences à activité unique** (le type d'activité n'est pas restauré par l'import, ces séquences se retrouvent alors avec une activité "forum" par défaut).
:::
